...
class BleTransport extends hw_transport_1.default {
    ...
    constructor(device, writeCharacteristic, writeCmdCharacteristic, notifyObservable, deviceModel, bluetoothInfos) {
         ...
         */
        this.exchange = async (apdu) => {
            return this.exchangeAtomicImpl(async () => {
                try {
                    const msgIn = apdu.toString("hex");
                    (0, logs_1.log)("apdu", `=> ${msgIn}`);
                    const data = await (0, rxjs_1.merge)(this.notifyObservable.pipe(receiveAPDU_1.receiveAPDU), (0, sendAPDU_1.sendAPDU)(this.write, apdu, this.mtuSize)).toPromise();
    ...
    async inferMTU() {
        ...
        await this.exchangeAtomicImpl(async () => {
            try {
                mtu = await (0, rxjs_1.merge)(this.notifyObservable.pipe((0, operators_1.tap)((maybeError) => {
                    if (maybeError instanceof Error)
                        throw maybeError;
                }), (0, operators_1.first)((buffer) => {
                    return buffer.readUInt8(0) === 0x08;
                }), (0, operators_1.map)((buffer) => buffer.readUInt8(5))), (0, rxjs_1.defer)(() => (0, rxjs_1.from)(this.write(Buffer.from([0x08, 0, 0, 0, 0])))).pipe((0, operators_1.ignoreElements)())).toPromise();
            }
            catch (e) {
 ...
 */
BleTransport.disconnect = async (id) => {
